...
function renderHeader(doc: PDFDocument.PDFDocument, value: unknown): void {
  ...
  doc
    .rect(MARGIN - 10, headerTop, CONTENT_WIDTH + 20, headerHeight)
    .fill(COLOR.headerBg);
...
function renderSection(doc: PDFDocument.PDFDocument, config: { title?: string }, value: unknown): void {
...
function renderTable(
  ...
  value.forEach((row, rowIndex) => {
    ...
    // Cell borders
    doc
      .rect(startX, currentY, CONTENT_WIDTH, rowHeight)
...
function drawBarChart(
  ...
  sampled.forEach((point, i) => {
    ...
    doc.rect(bx, by, barW, Math.min(barH * 0.25, 4)).fillColor('#60A5FA').fill();
    ...
    if (i % labelEvery === 0) {
      ...
      doc
        .fontSize(6.5)
        .fillColor(COLOR.textLight)
        .text(label, bx - 2, baseY + 5, { width: barW + 4, align: 'center', lineBreak: false });
    }
  });
...
function drawLineChart(
  ...
  coords.forEach((c, i) => {
    ...
    if (i % labelEvery === 0) {
      ...
      doc.fontSize(6.5).fillColor(COLOR.textLight).text(label, c.x - 15, baseY + 5, {
        width: 30,
        align: 'center',
        lineBreak: false,
      });
...
export default async function pdfWorker(input: WorkerInput): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < totalPages; i++) {
      ...
      const page = doc.page as { height: number; width: number; margins?: { top: number; bottom: number; left: number; right: number } };

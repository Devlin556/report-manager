...
function makeColumnBar(value: number, maxValue: number, width = 20): string {
    ...
    const idx = Math.min(
        Math.floor((value / maxValue) * SPARKLINE_CHARS.length),
...
function makeSparkline(values: number[], width = 20): string {
    ...
    const indices =
        values.length <= width
            ? values.map((v) =>
                  Math.min(
                      Math.floor((v / max) * (SPARKLINE_CHARS.length - 1)),
                      SPARKLINE_CHARS.length - 1,
                  ),
              )
            : values
...
function detectColumns(rows: Record<string, unknown>[], config: TemplateModuleConfig): string[] {
...
function renderChartSheet(
    ...
    const pctHeader =
        chartType === 'pie' || chartType === 'doughnut' ? '% от общего' : '% от макс.';
    const vizHeader =
        chartType === 'pie'
            ? 'Доля'
            : chartType === 'doughnut'
